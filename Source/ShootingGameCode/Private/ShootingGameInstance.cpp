...
void UShootingGameInstance::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			{
				TArray<FBlueprintSessionResult> arrSessionResult;
				arrSessionResult.SetNum(SessionSearch->SearchResults.Num());
				...
				for (int32 SearchIdx = 0; SearchIdx < SessionSearch->SearchResults.Num(); SearchIdx++)
				{
					...
					arrSessionResult[SearchIdx].OnlineResult = SessionSearch->SearchResults[SearchIdx];
				}

				OnUpdateSessionResult(arrSessionResult);
			}
		}
	}
}
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			if (PlayerController && Sessions->GetResolvedConnectString(SessionName, TravelURL))
			{
				...
				FString NewTravelURL = FString::Printf(TEXT("%s:%d"), *strIP, Port);

				GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red
					, FString::Printf(TEXT("TravelURL=%s"), *NewTravelURL));
...
void UShootingGameInstance::StartOnlineGame(int MaxPlayerNum, bool bIsLAN)
{
	...
	HostSession(UniqueNetId, GameSessionName, bIsLAN, true, MaxPlayerNum);
}
